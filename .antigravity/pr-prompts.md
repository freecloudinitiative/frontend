...
- **Intermediate Responsive Breakpoint Refinements (`769px` – `1450px`)**:
  ...
  - **Mobile Header Action Bar Restoration & Spacing (`<= 768px`)**: Restored the Connect action button (`btn-mobile-run`, `▶`) across all 5 services (`VM`, `Database`, `IAM`, `Storage`, `Network`) in `DashboardPage.tsx`. Configured `.fci-topbar-actions` in `tui-dashboard.css` with `justify-content: space-between` and responsive `gap: clamp(6px, 2.5vw, 12px)` for balanced distribution across primary action buttons, Region selector, and Profile dropdown.
